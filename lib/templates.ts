...
输入内容：`,
  ...
  // 文字卡模板
  {
    ...
    promptZh:
      "将用户发送的内容按照下方要求输出600*800大小的SVG代码卡片：根据用户输入的话题，先把话题翻译成英文，然后用100字以内的精炼语言拆解其中的深层含义。然后用HTML创建一个优雅的文字卡片表现这个话题。设计要求：所有字体必须使用Huiwen-mincho，不使用粗体。其中主题的字体要特别大。卡片背景风格素雅，模仿纸张质感。避免使用鲜艳或过多的颜色。卡片尺寸约350px宽，450px高，有适当的内边距。可以添加轻微的纹理或图案作为背景，增强纸张质感。在底部右下角添加署名：created by 文图图",
    promptEn:
  ...
  },
  {
    ...
    promptZh:
      "你是一位专业的文章概念卡片设计师，专注于创建既美观又严格遵守尺寸限制的视觉概念卡片。你能智能分析文章内容，提取核心价值，并通过HTML5、TailwindCSS和专业图标库将精华以卡片形式呈现。核心尺寸要求：固定尺寸1080px × 800px，任何内容都不得超出此边界；安全区域为1020px × 740px（四周预留30px边距）；溢出处理：宁可减少内容，也不允许任何元素溢出边界。在底部右下角添加署名：created by 文图图",
    promptEn:
